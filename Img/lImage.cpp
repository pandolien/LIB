...
void lImage::OrientedFAST(lPTRList* list, unsigned char th, int cnt)
{
	int i, j, k;
	int histogram[36];
	float mx,my;
	...
	for (i = 0; i < list->GetSize(); i++) {
		...
		for (j = 0; j < 31; j++) {
			for (k = 0; k < 31; k++) {
				int x = -15 + k;
				int y = -15 + j;
				if (pt->u + x < 0) continue;
				if (pt->u + x >= w) continue;
				if (pt->v + y < 0) continue;
				if (pt->v + y >= h) continue;
				float I = (float)(Out(pt->v + y, pt->u + x, pt->ch))/255;
				mx += x * I;
				my += y* I;
			 }
		}
		pt->angle = atan2(my, mx);
		pt->angle = DEG(pt->angle);
	}
